...
from students.forms import ChangeGroupForm
...
class StudentAdmin(admin.ModelAdmin):
    ...
    actions = ['make_krasivo', 'change_group', 'copy_student', "remove_group", ]
    ...
    def change_group(self, request, queryset):
        ...
        if 'do_action' in request.POST and form.is_valid():
            group = form.cleaned_data['group']
            updated_students = queryset.update(student_group=group)
            self.message_user(request,
                              u"У %s студентів було змненно групу на %s" % (
                                  updated_students, group))

            return
            # return HttpResponseRedirect(request.get_full_path())

        return render(request, 'students/admin_change_group.html',
                      {'students': queryset, 'form': form,
                       'title': u'Зміна групи'})

    change_group.short_description = u'Перемістити у групу'
    ...
    def remove_group(self, request, queryset):
        ...
        self.message_user(request,
                          "Group was changed. Student free for any group now.")

    remove_group.short_description = u"Удалить из группы"
...
class GroupFormAdmin(ModelForm):
    ...
    def clean_starosta(self):
        ...
        if self.cleaned_data['starosta'] in students or self.cleaned_data[
            'starosta'] is None:  # and len(students) > 0:
